        ...
        */
        ...
        switch (command) {
            case 'antiporn': // Premium, chat VideFikri
                await bocchi.reply(from, 'Premium Feature!\n\nContact: wa.me/6287833474586?text=Buy%20Anti%20Porn', id)
            break
            ...
            case 'register':
                ...
                if (!q.includes('|')) return await bocchi.reply(from, 'Silakan daftar dengan format:\n*!register* nama | umur', id)
                ...
                console.log(color('🔰REGISTER🔰'), color(time, 'yellow'), 'Nama:', color(namaUser, 'cyan'), 'Umur:', color(umurUser, 'cyan'), 'Kode Serial:', color(serialUser, 'cyan'))
            break
            ...
            case 'ytmp44':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                downloader.ytdll(url)
                    .then(async (res) => {
                        if (res.status === 'error') {
                            await bocchi.reply(from, res.result.title, id)
                        } else if (Number(res.result.size.split(' MB')[0]) >= 30) { 
                            await bocchi.reply(from, ind.videoLimit(), id)
                        } else {
                            await bocchi.sendFileFromUrl(from, res.result.imgUrl, `${res.result.title}.jpg`, ind.ytFound(res), id)
                            await bocchi.sendFileFromUrl(from, res.result.UrlVideo, `${res.result.title}.mp4`, '', id)
                            console.log('Success sending YouTube video!')
                        }
                    })
                    .catch(async (err) => {
            ...
            case 'linedl': // chikaa chantexxzz
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await bocchi.reply(from, ind.pcOnly(), id)
                if (!isUrl(url) && !url.includes('store.line.me')) return await bocchi.reply(from, 'Ketik !linedl (linkstickerline)', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                downloader.line(url)
                    .then(async (res) => {
            ...
            case 'googlesearch':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'masukkan apa yang ingin kamu cari di google\n*Contoh:*\n!google Mata Uang Uganda', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
            ...
            case 'afk': // by Slavyan
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const reason = q ? q : 'Tidak diketahui.'
            ...
            case 'jadwalshalat':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Contoh: !jadwalshalat Trenggalek', id)
                const jds = await get.get(`https://tobz-api.herokuapp.com/api/jadwalshalat?q=${q}&apikey=BotWeA`).json()
                if (jds.error) {
                    await bocchi.reply(from, jds.error, id)
                } else {
                    const { imsak, subuh, dzuhur, ashar, maghrib, isha } = jds.result
                    ...
                    const txt = `Jadwal sholat di *${q}*\n${tgl}-{arr[bln]}-${thn}\n\nSubuh: ${subuh}\nDzuhur: ${dzuhur}\nAshar: ${ashar}\nMaghrib: ${maghrib}\nIsya: ${isha}`
                    await bocchi.reply(from, txt, id)
                }
            break
            /*case 'hidetag':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Masukkan pertanyaan kamu senpai', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
            ...
            case 'brainly':
                ...
                misc.brainly(q)
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, `${q} tidak ditemukan di brainly`, id)
                    })
            break
            case 'wikipedia':
            case 'wiki':
                ...
                misc.wiki(q)
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from,  `${q} tidak ditemukan di wikipedia`, id)
                    })
            break
            ...
            case 'ttp': // CHIKAA CHANTEKKXXZZ
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik !ttp (text) untuk merubah text menjadi sticker', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                misc.ttp(q)
            ...
            case 'genshin':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Contoh: !genshininfo keqing', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                try {
            ...
            case 'igstory':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return bocchi.reply(from, 'Ketik: !instastory (link_instastory)', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await bocchi.sendFileFromUrl(from, urlDownload, '', 'Instastory telah di download senpai', id)
            ...
            case 'kbbi':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik kosakata yang ingin dicari di KBBI', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                misc.kbbi(q)
            ...
            case 'cuaca':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik !cuaca (daerah)\nContoh: !cuaca Trenggalek', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                misc.kbbi(q)
            ...
            case 'jadwalsalatt':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'ketik !jadwalsholat (daerah)', id)
                await bocchi.reply(from, ind.wait(), id)
                misc.jadwalSholat(q)
                    .then((data) => {
            ...
            case 'ssweb':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return bocchi.reply(from, 'Kirim perintah *!url2img [web]*\nContoh *!url2img nekopoi.care*', id)
                const url2img = await get.get(`https://api.vhtear.com/ssweb?link=${q}&type=phone&apikey=0504032003slavyan`)
                if (url2img.error) return bocchi.reply(from, url2img.error, id)
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/ssweb?link=${q}&type=phone&apikey=0504032003slavyan`, 'kyaa.jpg', null, id)
            break
            case 'nulis':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return bocchi.reply(from, 'Kirim perintah *!nulis text*\nContoh *!nulis saya malas nulis*', id)
                const nulis = await get.get(`https://api.vhtear.com/write?text=${q}&apikey=0504032003slavyan`)
                if (nulis.error) return bocchi.reply(from, nulis.error, id)
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${q}&apikey=0504032003slavyan`, 'kyaa.jpg', null, id)
            break
            case 'igstalk':
                ...
                if (!q) return await bocchi.reply(from, 'Kirim perintah *!igStalk @username*\nConntoh *!igStalk @mia_khalifa*', id)
                const stalk = await get.get(`https://api.vhtear.com/igprofile?query=${q}&apikey=${config.vhtear}`).json()
                if (stalk.error) return bocchi.reply(from, stalk.error, id)
                const caps = `⚜ *Nama* : ${stalk.result.full_name}\n⚜ *Username* : ${stalk.result.username}\n⚜ *Jumlah Followers* : ${stalk.result.follower}\n⚜ *Jumlah Following* : ${stalk.result.follow}\n⚜ *Jumlah Postingan* : ${stalk.result.post_count}\n⚜ *Biodata* : ${stalk.result.biography}`
                await bocchi.sendFileFromUrl(from, stalk.result.picture, 'Profile.jpg', caps, id)
            break
            case 'gsmarena':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik Merk HP yang mau dicari spec nya\nContoh: !gmsarena POCO X3 NFC', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                try {
            ...
            case 'resep':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik masakan apa yang mau diketahui resep nya', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                try {
            ...
            case 'findstiker':
                ...
                try {
                    ...
                    console.error(err)
                    await bocchi.reply(from, `Maaf Senpai,Command ini masih dalam perbaikan!\n\n${err}`, id)
                }
            break
            case 'movie':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
           ...
           /* case 'nhtest':
              ...
              const today = await get.get("https://api.vhtear.com/nhentairandom&apikey=0504032003slavyan").json()
              if (today.error) {
                  client.reply(from, today.error, id)
              } else {
                  for (let i = 0; i < today.result.length; i++) {
                      const { images } = today.result[i]
                      const txt = `== ${today.result.title} ==\n\n*Rilis*: ${today.result.secondary_title}==\n\n*tags*: ${today.result.secondary_title}`
                      await client.sendFileFromUrl(from, images, 'anime.jpg', txt)
                  }
              }*/
            break
            case 'ytsearch':
            ...
            case 'tts':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Kirim perintah *!tts [id, en, jp, ar] [teks]*, contoh *!tts id pagi lord*', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'groupinfo':
                ...
                let groupPic = await bocchi.getProfilePicFromServer(chat.id)
                ...
                await bocchi.sendFileFromUrl(from, pfp, 'group.jpg', `*${groupName}*\n\n👥 *Member: ${totalMem}*\n🗒️ *Deskripsi grup*:\n${groupDesc}`, null, null, true)
            ...
            case 'img2url':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await bocchi.reply(from, 'Upload Foto dengan caption !img2url', id)
                }
            break
            case 'infohoax':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'spamcall':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return bocchi.reply(from, 'masukkan nomor target tanpa 62.\nContoh: !spamcall 8783347xxxx', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                misc.call(q)
            ...
            case 'jatidiri':
                if (!isRegistered) return await bocchi.reply(from, 'Nomor kamu belum terdaftar di database! Silakan daftar dengan format:\n*!register* <nama | daerah>', id)
                if (!q) return await bocchi.reply(from, 'Kirim perintah *!jatidiri [nama]*\nContoh : *!jatidiri izumi*', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const pas_ = body.slice(6)
                const sip_ = body.slice(6)
                const jodoh = await get.get(`https://arugaz.herokuapp.com/api/jodohku?nama=${q}&pasangan=${q}`).json()
                if (jodoh.error) {
                    bocchi.reply(from, jodoh.error, id)
                } else {
            ...
            case 'menu':
            case 'help':
                //const limit = limit.getLimit(sender.id, _limit, limitCount)
                ...
                if (args[0] === '1') {
                    ...
                    await bocchi.sendText(from, ind.menu(jumlahUser, levelMenu, xpMenu, role, pushname, reqXpMenu, isPremium ? 'Premium User 💎' : 'Membership User'), id)
                }
            break
            case 'rules':
            case 'snk':
            ...
            case 'nsfw':
                ...
                if (ar[0] === 'enable') {
                    ...
                    await bocchi.reply(from, 'pilih Enable atau Disable', id)
                }
            break
            case 'botstat':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'runtime': // BY HAFIZH
                ...
                await bocchi.reply(from, `── *🔰 IZUMI-BOT UPTIME 🔰* ──\n\n ❏${formater(uptime)}`, id)
            break
            case 'ping':
            case 'p':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'bug':
                ...
                if (lastReport !== undefined && cd1 - (Date.now() - lastReport) > 0) {
                    const time = ms(cd1 - (Date.now() - lastReport))
            ...
            case 'reqpremium':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, ind.emptyMessPremium(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const lastPremium = daily.getLimit(sender.id, _daily)
                if (lastPremium !== undefined && cd1 - (Date.now() - lastPremium) > 0) {
                    const time = ms(cd1 - (Date.now() - lastPremium))
                    await bocchi.reply(from, ind.daily(time), id)
                } else {
                    if (isGroupMsg) {
                        await bocchi.sendText(ownerNumber, `-----[ Permintaan Premium ]-----\n\n*From*: ${pushname}\n*ID*: wa.me/${sender.id}\n*Group*: ${(name || formattedTitle)}\n*Message*: ${q}`)
                        await bocchi.reply(from, ind.receivedpremium(pushname), id)
                    } else {
                        await bocchi.sendText(ownerNumber, `-----[ Permintaan Premium ]-----\n\n*From*: ${pushname}\n*ID*: wa.me/${sender.id}\n*Message*: ${q}`)
                        await bocchi.reply(from, ind.receivedpremium(pushname), id)
                    }
            ...
            case 'join':
                ...
                if (!isUrl(url) && !url.includes('chat.whatsapp.com')) return await bocchi.reply(from, 'Ketik !join link grub', id)
                const checkInvite = await bocchi.inviteInfo(url)
                if (isOwner) {
                    await bocchi.joinGroupViaLink(url)
                    await bocchi.reply(from, ind.ok(), id)
                    await bocchi.sendText(checkInvite.id, `Hello!! Aku telah diundang oleh ${pushname}`)
                } else {
                    const getGroupData = await bocchi.getAllGroups()
                    if (getGroupData.length >= groupLimit) {
                        await bocchi.reply(from, `Sorry senpai, group nya telah mencapai: ${groupLimit}`, id)
                    } else if (getGroupData.size <= memberLimit) {
                        await bocchi.reply(from, `Kalo mau ngundang bot ke grub,minimal member harus: ${memberLimit}`, id)
                    } else {
                        if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                        limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        await bocchi.joinGroupViaLink(url)
                        await bocchi.reply(from, ind.ok(), id)
                        await bocchi.sendText(checkInvite.id, `Hello!! saya diundang oleh ${pushname}`)
                    }
                }
            break
            case 'premiumcheck':
            case 'cekpremium':
                ...
                await bocchi.reply(from, `*🔰 PREMIUM STATUS 🔰*\n\n➸ *ID*: ${sender.id}\n➸ *Premium berakhir pada*: ${cekExp.days} hari ${cekExp.hours} jam ${cekExp.minutes} menit`, id)
            break
            case 'premiumlist':
            case 'listpremium':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            break
            case 'limit':
                if (isPremium || isOwner) return await bocchi.reply(from, '⤞ Hit left: ∞ (UNLIMITED)', id)
                await bocchi.reply(from, `⤞ Hit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 25\n\n*_Limit direset pada pukul 00:00 WIB_*`, id)
            break
            ...
            case 'wait':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await bocchi.reply(from, 'Upload screenshot cuplikan anime dengan caption !wait', id)
                }
            break
            case 'source':
            case 'sauce':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await bocchi.reply(from, `upload foto yang ingin kamu cari source nya dengan caption !sauce`, id)
                }
            break
            case 'loli':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                const loli = await get.get(`https://tobz-api.herokuapp.com/api/randomloli?apikey=BotWeA`).json()
                if (loli.error) return bocchi.reply(from, loli.error, id)
                const res_loli = `*PUKUL MUNDUR PEDO....*`
                await bocchi.sendFileFromUrl(from, loli.result, 'loli.jpg', res_loli, id)
            break
            case 'waifu':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
            ...
            case 'pasangan':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'masukkan ( nama kamu | pasangan kamu )', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'nulis':
                ...
                await bocchi.sendFileFromUrl(from, `http://api-melodicxt-3.herokuapp.com/api/joki-nulis?text=${q}&apikey=administrator`, 'nulis.jpg', '', id)
            ...
            case 'fflogo': // By: VideFrelan
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return bocchi.reply(from, 'Jangan lupa masukkan nama karakter Free Fire\n\nContoh: *!fflogo Alok | Free Fire Game Burik*\n\nNama karakter FF yang tersedia:\n*Alok,Andrew,Ford*' , id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                ...
                console.log('Success!')
            break
            case 'avatarnowm': // By: VideFrelan
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                if (!q.includes('|')) return bocchi.reply(from, 'Contoh: *!avatarnowm KOWE | JANCUK*' , id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating wolf logo...')
                const text1 = q.substring(0, q.indexOf('|') - 1)
                const textrandom = q.substring(q.lastIndexOf('|') + 2)
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/avatarwolf?text1=${text1}&text2=${textrandom}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'avatar':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Contoh: *!avatar REZA ARAP GG GEMINK*', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, 'Silahkan tunggu sebentar...\nAvatar sedang di build\n\nUntuk versi No Watermark ketik *!avatarnowm*', id)
            ...
            case 'logo':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                if (!q) return await bocchi.reply(from, 'Ketik: !logo EVOS', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating Logo')
                await bocchi.sendFileFromUrl(from, `https://api.vhtear.com/gamelogo?text=${q}&apikey=${config.vhtear}`, id)
                console.log('Success creating Logo!')
            break
            case 'darkneon':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik: !darkneon text', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating Darkneon')
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/darkneon/?text=${q}`, id)
                console.log('Success creating Logo!')
            break
            case 'coffe':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik: !coffe text', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating Coffe')
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/coffeecup2/?text=${q}`, id)
                console.log('Success creating Coffe!')
            break
            case 'naruto':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'Ketik: !naruto text', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                console.log('Creating naruto')
                await bocchi.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/narutobanner/?text=${q}`, id)
                console.log('Success creating naruto!')
            break
            case 'say': // by: VideFrelan
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await bocchi.reply(from, ind.groupOnly(), id)
                if (!q) return await bocchi.reply(from, 'ketik !say apa kabar?', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                fun.simi(q)
                    .then(async ({ jawaban }) => {
            ...
            case 'pornhublogo':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, 'ketik !pornhublogo ( text1 | text2 )', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'weton':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return await bocchi.reply(from, 'Maaf Senpai,Format yang anda masukkan salah\nContoh: !weton 01 | 03 | 2002 ', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                fun.weton(tgl, bln, thn)
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await bocchi.reply(from, 'Maaf Senpai,Format yang anda masukkan salah\nContoh: !weton 01 | 03 | 2002 ', id)
                    })
            break
            case 'truth':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
            ...
            case 'kiss':
                ...
                try {
                    if (isMedia && isImage || isQuotedImage) {
                        ...
                        await bocchi.reply(from, 'upload foto pasangan kamu dengan caption !kiss', id)
                    }
                } catch (err) {
                    console.error(err)
                    await bocchi.reply(from, 'Maaf Senpai,Command ini masih dalam perbaikan!', id)
            ...
            case 'sliding':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
                await bocchi.sendVideoAsGif(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=${config.vhtear}`, 'sliding.gif', '', id)
            ...
            case 'text': // by: irham01
                ...
                try {
                    if (ar[0] === 'burnpaper') {
                        ...
                        await bocchi.sendFileFromUrl(from, `${q}.jpg`, '', id)
            ...
            case 'stcmeme':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return await bocchi.reply(from, 'Upload foto dengan caption !stickermeme text1 | text2', id)
                if (isMedia && isImage || isQuotedImage) {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
            ...
            case 'stiker':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (isMedia && isImage || isQuotedImage) {
                    await bocchi.reply(from,'[ WAIT ] Sticker sedang di build⏳ silahkan tunggu sebentar ya senpai 🍻', id)
                    const encryptMedia = isQuotedImage ? quotedMsg : message
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    webp.buffer2webpbuffer(mediaData, 'jpg', '-q 100')
                        .then((res) => {
                            sharp(res)
                                ...
                                .toFile(`./temp/stage_${sender.id}.webp`, async (err) => {
                                    ...
                                    if (fs.existsSync(`./temp/${sender.id}.webp`)) {
                                        const data = fs.readFileSync(`./temp/${sender.id}.webp`)
                                        const base64 = `data:image/webp;base64,${data.toString('base64')}`
                                        await bocchi.sendRawWebpAsSticker(from, base64, id)
                                        await bocchi.reply(from, ind.ok(), id)
                                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                                        fs.unlinkSync(`./temp/${sender.id}.webp`)
                                        fs.unlinkSync(`./temp/stage_${sender.id}.webp`)
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, '[ WAIT ] Sticker Gif sedang di build⏳ silahkan tunggu sebentar ya senpai 🍻', id)
                    try {
            ...
            case 'ttg':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                if (!q) return await bocchi.reply(from, '[ WAIT ] Sticker GIF sedang di build⏳ silahkan tunggu sebentar ya senpai 🍻', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.reply(from, ind.wait(), id)
            ...
            case 'nhsearch':
                ...
                if (isGroupMsg) {
                    ...
                    nana.search(q)
                        ...
                        .catch(async (err) => {
                            console.error(err)
                            await bocchi.reply(from, 'Maaf Senpai,yang anda cari tak bisa saya temukan', id)
                        })
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            break
             case 'furry':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                const kucing = await get.get('https://tobz-api.herokuapp.com/api/nsfwneko?apikey=BotWeA').json()
                if (kucing.error) return bocchi.reply(from, kucing.error, id)
                const res_kucing = `*PUKUL MATI FURRY*`
                bocchi.sendFileFromUrl(from, kucing.result, 'nekoo.jpg', res_kucing, id)
             } else {
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            break
             case 'trap':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await bocchi.reply(from, ind.wait(), id)
                const kucing = await get.get('https://tobz-api.herokuapp.com/api/nsfwtrap?apikey=BotWeA').json()
                if (kucing.error) return bocchi.reply(from, kucing.error, id)
                const res_kucing = `*KONTOOL!!!*`
                bocchi.sendFileFromUrl(from, kucing.result, 'nekoo.jpg', res_kucing, id)
             } else {
                    if (!isPremium) return await bocchi.reply(from, ind.notPremium(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
           ...
           /*case 'xnxx':
                ...
                try {
                    misc.xnxx(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                const { title, url, duration, image } = await result.data[i]
                                await bocchi.sendFileFromUrl(from,  ind.`Title: ${title}\nURL: ${url}\nDuration: ${duration}\n`(url, title, duration), id)
                                console.log('Success sending xnxx results!')
                            }
                        })
                } catch (err) {
                    console.error(err)
            ...
            case 'xnxx':
              ...
              if (xnxxget.error) {
                  client.reply(from, xnxxget.error, id)
              } else {
            ...
            case 'add':
                ...
                if (!isGroupAdmins) return await bocchi.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await bocchi.reply(from, ind.botNotAdmin(), id)
                if (args.length !== 1) return await bocchi.reply(from, `ketik !add 0878xxxxxxx`, id)
                try {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'kick':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await bocchi.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await bocchi.sendTextWithMentions(from, `Sayounara Senpai~ 👋\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            ...
            case 'demote':
                ...
                if (!isGroupAdmins) return await bocchi.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await bocchi.reply(from, ind.botNotAdmin(), id)
                if (mentionedJidList.length !== 1) return await bocchi.reply(from, `ketik !add 0878xxxxxxx`, id)
                if (mentionedJidList[0] === botNumber) return await bocchi.reply(from, ind.wrongFormat(), id)
            ...
            case 'listadmin':
                ...
                if (lastAdmin !== undefined && cd1 - (Date.now() - lastAdmin) > 0) {
                    const time = ms(cd1 - (Date.now() - lastAdmin))
                    await bocchi.reply(from, ind.daily(time), id)
                } else if (isOwner) {
                    let txt = '╔══✪〘 *ADMIN* 〙✪══\n'
            ...
            case 'mentionall':
                ...
                if (lastEveryone !== undefined && cd1 - (Date.now() - lastEveryone) > 0) {
                    const time = ms(cd1 - (Date.now() - lastEveryone))
                    await bocchi.reply(from, ind.daily(time), id)
                } else if (isOwner) {
                    let txt = ' 🔰 SUMMON MEMBER 🔰 \n'
            ...
            case 'unreg':
                if (!isRegistered) return
                const indx = register.getRegisteredUserPosition(sender.id, _registered)
                _registered.splice(indx, 1)
                fs.writeFileSync('./database/bot/registered.json', JSON.stringify(_registered))
                await bocchi.reply(from, ind.doneUnreg(), id)
            break
            ...
            case 'editor':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, ind.editor())
            break
            case 'menuowner':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, ind.owner())
            break
            case 'nsfwlist':
                if (!isRegistered) return await bocchi.reply(from, ind.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, ind.nsfww())
            break
        }
    } catch (err) {
