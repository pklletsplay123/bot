...
/**
 * Get Brainly result from given query.
 * @param {string} query
 * @returns {Promise<object>}
 */
const brainly = (query) => new Promise((resolve, reject) => {
    console.log(`Get result for ${query} in brainly...`)
    fetchJson(`https://api.vhtear.com/branly?query=${query}&apikey=0504032003slavyan`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
...
const xnxx = (query) => new Promise((resolve, reject) => {
    console.log(`Get xnxx search results for ${query}...`)
...
/**
 * Get Cuaca search results.
 * @param {string} query 
 * @returns {Promise<object>}
 */
const weather = (query) => new Promise((resolve, reject) => {
    console.log(`Get Play Store search results for ${query}...`)
 ...
 * @returns {Promise<object>}
 */
const sswebphone = (title) => new Promise((resolve, reject) => {
    console.log(`Searching for ssweb phone ${title}...`)
    fetchJson(`https://api.vhtear.com/ssweb?link=${title}&type=phone&apikey=${config.vhtear}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
