 ...
 * @returns {Promise<object>}
 */
const nekopoii = (query) => new Promise((resolve, reject) => {
    console.log(`Get Nekopoi search results for ${query}...`)
    fetchJson(`https://api.vhtear.com/nekohentai&apikey=${config.vhtear}`)
        .then((result) => resolve(result))
        .catch((err) => reject(err))
})
